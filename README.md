...
Current track
    ↓
Music Fingerprint + Aesthetic Reading
(sound / emotion / human state / tension / world / unfinished expression)
    ↓
Constraint-aware recall directions
    ↓
NetEase real candidate recall
    ↓
Listening Judgment
(worthiness / meaningful difference / surprise / obviousness / cliché risk)
    ↓
Deterministic aesthetic gate
(diversity / world breaks / anti-lazy first step)
    ↓
Listening Arc
...
Similarity can make a candidate technically correct and still aesthetically lazy. From Here prefers a meaningful continuation, opening or turn — surprise under constraints — and judges the sequence as a listening arc, not five isolated winners.
...
Download `From-Here-v1.1.0-macOS.zip` from **GitHub Releases** instead of GitHub's autogenerated Source ZIP.

The official Release is a **self-contained runtime** for both Apple Silicon and Intel Macs. It already carries the Node runtime and the official `@music163/ncm-cli` package inside its hidden `.from-here/` directory. It does not install Homebrew, does not require npm, and does not download a separate Now Playing helper during setup.

The visible package stays small conceptually:
...
1. Double-click `Install.command` and complete the guided NetEase authorization / AI setup.
...
From Here 1.1 reads the current macOS song through the system `/usr/bin/osascript` process and a bundled JXA script using `MRNowPlayingRequest`. There is **no Homebrew / GHCR / nowplaying-cli download path** in the normal installation flow.

For NetEase itself, the platform still requires each user to prepare an Open Platform `App ID` + `PrivateKey` once. From Here explains this before handing off to the official ncm-cli setup, and the credentials remain managed locally by ncm-cli. From Here does **not** read, store or upload them.

If you already configured the same developer credentials on another Mac, you can reuse them; only the local ncm-cli authorization/login state must be completed on the new machine.

You can skip NetEase authorization during first install and resume later with `Support/Connect NetEase.command`.

### If you cloned the source repository

A source checkout is a developer environment, so it still expects:
...
No separate macOS media helper is required. `Install.command` verifies the built-in system media adapter directly.
...
Regression coverage includes provider adapters, local-model priority, media-state persistence, unresolved Anchor IDs, constraint-aware recall, feedback-without-skipping, rolling auto-refill, the Dirty Paws tribute bad case, perceptual world-break protection, anti-obviousness, trajectory composition and AI Listening Judgment.
...
bridge/server.js
  Session orchestration
       ...
       └── system JXA adapter    current macOS track
```
...
A local developer build uses the runtimes already present on the machine. Official GitHub Releases are built by `.github/workflows/release.yml`: the workflow pins and verifies official Apple Silicon + Intel Node binaries, installs a pinned `@music163/ncm-cli`, bundles both architectures into one macOS ZIP, then smoke-tests that exact artifact on real arm64 and Intel GitHub-hosted macOS runners before publishing it.

The release builder strips tests, CI, design docs and source-only files, hides the runtime Bridge, and rejects local secrets/cache before producing the ZIP.
...
**v1.1.0 — Listening Judgment + self-contained macOS runtime.** From Here now asks whether a real candidate *deserves to be next*, composes a listening arc instead of five isolated winners, lets natural-language boundaries reshape recall, and decouples the current Anchor from catalog playability. The normal user install also no longer depends on Homebrew, GHCR, a downloaded Now Playing binary, a preinstalled Node.js, or a global ncm-cli.
